...
# parameters to be defined to make the code run on any system without path adaptation
...
def load_filepaths_and_text(filelist, split="|"):
    if isinstance(filelist, str):
        with open(filelist, encoding='utf-8') as f:
            filepaths_and_text = [os.path.join(f'{siddha_param.dp_data}', line) for line in f]
            filepaths_and_text = [line.strip().split(split) for line in filepaths_and_text]
    else:
...
def load_wav_to_torch(full_path):
    ...
    # resampling the same audio file using librosa and scipy will result in a different resampled audio output because librosa:
        # Load an audio file as a floating point time series.
        # will automatically resample the input audio to the given rate (default sr=22050).
        # will not preserve the native sampling rate of the input audio file (to do so, use sr=None).
    # whereas scipy:
        # does not Load an audio file as a floating point time series if they are integers in the file.
        # does not automatically resample the data.
        # ...
    # to change `librosa` output to match `scipy`:
